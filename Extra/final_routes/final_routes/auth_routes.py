...
def add_user(user: User):
    ...
    values = (user.name, user.phone, user.email, user.role, hashed_pw)
...
def admin_create_user(name: str = Form(...), email: str = Form(...), phone: str = Form(...),
                      role: str = Form(...), password: str = Form(...), current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "Admin":
        raise HTTPException(status_code=403, detail="Not authorized")
    hashed_pw = hash_password(password)
    try:
        execute_query("INSERT INTO users (name, phone, email, role, password) VALUES (%s, %s, %s, %s, %s)",
