...
async def handle_list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            name="write_file",
            description="Write or update a file in the application directory",
            inputSchema={
                "type": "object",
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Relative path from application root (e.g., 'routes/new_route.py', 'static/new_page.html')"
                    },
                    "content": {
                        "type": "string",
    ...
    try:
        if name == "query_database":
            ...
            if create_dirs:
                ...
                raise ValueError(f"Parent directory does not exist: {os.path.dirname(path)}")
            ...
            return [TextContent(
                type="text",
                text=f"File written successfully: {path} ({len(content)} bytes)"
            )]
